...
COLLECTION_ADDRESS=$(everdev contract deploy Collection.abi.json -n $NWK -s $SIGNER -v 5000000000 -d _nonce:$NONCE -i "{\"codeNft\": \"$NFT_CODE\", \"codeIndex\": \"$INDEX_CODE\", \"codeIndexBasis\": \"$INDEX_BASIS_CODE\", \"ownerPubkey\":\"$PUBKEY\", \"json\": \"$JSON\", \"mintingFee\":0}" | grep "Address:" | cut -d " " -f 4)
